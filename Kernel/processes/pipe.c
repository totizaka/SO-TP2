...
pipe_t pipe_array[MAX_PIPES] ; // Array de pipes 
...
int initialize_pipes(){ //inicializo todos? 
    ...
    for ( i = 0; i < MAX_PIPES; i++) {
        pipe_array[i].id = i;
        pipe_array[i].pids[PIPE_READ] = -1;     
        pipe_array[i].pids[PIPE_WRITE] = -1;    
...
int64_t write_pipe( int64_t id, char* buffer, uint64_t num_bytes ){
    ...
    while (written< num_bytes){
        ...
        if(my_sem_wait(pipe_array[id].sem_write)==-1){ 
            return written;
        }
